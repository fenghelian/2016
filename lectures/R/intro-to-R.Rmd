...
```{r,eval=FALSE}
data()# a function shows all the data sets
```
...
```{r,eval=FALSE}
library(survival)#without cotes
```
...
```{r, warning=FALSE}
library(dplyr)# or equally use require()
```
...
args(plot)##give u the arguments the function needs
```
...
<<<<<<< HEAD:lectures/R_for_Data_Science/introToR.Rmd
...
>>>>>>> course/master:lectures/R/intro-to-R.Rmd
