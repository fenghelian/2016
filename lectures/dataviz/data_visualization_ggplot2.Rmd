...
```{r}
qplot(x = gdpPercap, y = lifeExp, data = filter(gapminder, year==2007),xlab = "GDPperCap",ylab = "LifeExp",main="GDP")
```
...
```{r}
p <- ggplot(filter(gapminder, year==2007), aes(x=gdpPercap)) #gdpPercap as x 
p +geom_histogram(bins=50)# prints the object
#alternatively could assign and print:p2<-p+geom_histogram then p2
```
...
```{r}
p <- ggplot(filter(gapminder, year==2007), aes(gdpPercap)) + geom_histogram(bins=25) + scale_x_log10()#one more means 10 times richer
p
```
...
```{r}
ggplot(filter(gapminder, year==2007), aes(x=gdpPercap, y = lifeExp)) +geom_point()+scale_x_log10() 
```
...
p + geom_point(aes(color=continent)) + scale_x_log10()#if each color gets a color it goes into aes()
```
...
Assessment: Examine the relationship for years 1910, ..., 2000. Use color to distinguish the continents.
```{r}
years=c(1967:2007)
p<-ggplot(filter(gapminder,year %in%years),aes(x=gdpPercap,y=lifeExp))
p+geom_point(aes(color=continent))+facet_wrap(~year,scale="free")+scale_x_log10()
```

Assessment. The points seem a bit small. Read the cheat sheet and learn how to increase the size of points.
```{r}
p+geom_point(aes(color=continent),size=4)+facet_wrap(~year,scale="free")+scale_x_log10()
```
