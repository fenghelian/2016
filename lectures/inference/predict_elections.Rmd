...
tabEst<-
  ...
  filter(n_polls>=5)%>%
  ungroup()%>%
  summarize(tot_poll=n(),theta_hat=mean(diff),sigma_hat=sd(diff))
```
...
tab1<-tab%>%
  ...
  filter(n_polls>=5)%>%
  ungroup()
sigma_empirical<-  sd(tab1$diff,na.rm=T)
```
...
```{r}
##Your code here
q-pnorm(0,tabEst$theta_hat,tabEst$sigma_hat/sqrt(tabEst$tot_poll))
...
tab %>% 
    filter(week>-5, N == 1000) %>%
    summarize(theta_hat=mean(diff),sigma_hat=2*sqrt(mean(Obama)*(1-mean(Obama)/1000)))
```
...
b<-1/tau_prior^2/(tabEst$tot_poll/tabEst$theta_hat^2+1/tau_prior^2)
e<-b*mu_prior+(1-b)*tabEst$theta_hat
var<-1/(tabEst$tot_poll/tabEst$theta_hat^2+1/tau_prior^2)
