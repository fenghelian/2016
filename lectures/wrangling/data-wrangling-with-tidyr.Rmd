...
dem_polls %>% 
    gather(key = candidate, value = percentage, -(Pollster:`End Date`))
```
...
```{r}
## Provide your code here
dem_polls%>% gather(key=candidate,value=percentage,Clinton:Undecided)%>%filter(candidate %in% c('Clinton','Sanders','O\'Malley')&'End Date'>="2015-05-01")
```
...
## Provide your code here
library(ggplot2)
dem_polls%>% gather(key=candidate,value=percentage,Clinton:Undecided)%>%filter(candidate %in% c("Clinton","Sanders","O\'Malley")&'End Date'>="2015-05-01")%>%ggplot(aes(x='End Date',y=percentage,color=candidate))+geom_line()
```
...
```{r}
## Provide your code here
dem_polls_united<-dem_polls_separate%>% unite(col='End Date',y,m,d,sep='-')
```
