...
msleep%>%select(-(genus:conservation))%>%filter(sleep_total>16&bodywt>=1)
...
## Provide your code here
library(dplyr)
msleep %>% mutate('rem_proportion'=sleep_rem/sleep_total)%>%ggplot(aes(x=vore,y=rem_proportion,fill=vore))+geom_boxplot()+xlab('Type of diet')+ylab('proportion')
```
...
```{r}
## Provide your code here
msleep%>%select(starts_with("sl"),ends_with("wt"))%>%mutate('rem_proportion'=sleep_rem/sleep_total,'bodywt_grams'=bodywt*1000)%>%slice(20:30)
```
...
```{r}
## Provide your code here
msleep%>%select(contains('sleep'))%>%arrange(desc(sleep_rem))
```
...
```{r}
## Provide your code here
msleep%>%select(name,order,sleep_total)%>%arrange(desc(sleep_total))%>%filter(sleep_total>=16)
```
...
## Provide your code here
msleep %>% 
    summarise(avg_sleep = mean(sleep_total),max_sleep=max(sleep_total),min_sleep=min(sleep_total),total=n())
```
...
msleep %>% 
    group_by(vore) %>%
    summarize("bodywt_avg" = mean(bodywt), 
              "brainwt_avg" = mean(brainwt, na.rm = TRUE))
```
...
Ans: it will only return the columns from the x table
...
```{r}
## Provide your code here
<<<<<<< HEAD:lectures/R_for_Data_Science/data_wrangling_with_dplyr.Rmd
left_join(oscars,movies)
# or
right_join(movies,oscars)
=======
...
>>>>>>> course/master:lectures/wrangling/data-wrangling-with-dplyr.Rmd
```
...
## Provide your code here
<<<<<<< HEAD:lectures/R_for_Data_Science/data_wrangling_with_dplyr.Rmd
right_join(oscars,movies)
# or
left_join(movies,oscars)
=======
